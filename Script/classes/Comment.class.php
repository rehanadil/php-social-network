...
class Comment {
	...
	public function putNotification($action)
	{
		...
		if ($action == "like")
		{
		    ...
		    {
		    	$this->getConnection()->query("INSERT INTO " . DB_NOTIFICATIONS . " (timeline_id,active,notifier_id,post_id,text,time,type,url) VALUES (" . $this->data['timeline']['id'] . ",1," . $user['id'] . "," . $this->id . ",'$text'," . time() . ",'like','index.php?a=story&id=" . $this->data['post_id'] . "#comment_" . $this->id . "')");
		    }
	...
	public function getTemplate() {
		...
		/* Image View */
		$themeData['comment_image_view'] = $this->getImageViewTemplate();
		...
		/* Return template */
		$this->template = \SocialKit\UI::view('comment/content', 'comment_content_ui_editor', $this->conn, $this->data);
		$this->template = \SocialKit\UI::view('comment/content', array(
            'key' => 'comment_content_ui_editor',
            'return' => 'string',
            'type' => 'APPEND',
            'content' => array(
            	'comment' => $this->data
	...
	public function getBasicTemplate() {
		...
		$themeData['comment_time'] = $this->data['time'];
		...
		$themeData['comment_timeline_link'] = \SocialKit\UI::view('comment/timeline-link', array(
        	'key' => 'comment_timeline_link_ui_editor',
        	'return' => 'string',
        	'type' => 'APPEND',
        	'content' => array(
        		'comment' => $this->data
	...
	public function getImageViewTemplate()
	{
		global $themeData;
		$themeData['comment_photo_url'] = "";

		if ($this->data['photo'] != false)
		{
			$themeData['comment_photo_url'] = $this->data['photo']['each'][0]['url'];
			return \SocialKit\UI::view('comment/image-view');
		}
