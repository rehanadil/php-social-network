...
class registerStory
{
    ...
    public function register()
    {
        ...
        if (!$this->timeline) $this->continue = false;
        if ($user['subscription_plan']['post_new_stories'] == 0) return false;

        if (empty($this->privacy))
        {
            $this->privacy = $config['story_default_privacy'];
        }

        $this->privacy = str_replace('following', 'friends', $this->privacy);
        ...
        if ($this->continue)
        {
            ...
            if ($query)
            {
                ...
                if ($config['story_allow_addons'] == 1)
                {
                    ...
                    {
                        \SocialKit\Addons::invoke(array(
                            'key' => 'new_story_insert_data',
                            'return' => "NONE",
                            'type' => "NO_APPEND",
                            'content' => array(
                ...
                if (is_post_add_data() == false)
                {
                    if (empty($this->text) && $this->mediaExists != true)
                    {
                        $this->getConnection()->query("DELETE FROM " . DB_POSTS . " WHERE id=" . $this->storyId);
                        return false;
                    }
                }
                ...
                if ($this->recipientId > 0 && $this->recipient['type'] == "user")
                {
                    if ($this->recipient['mailnotif_timelinepost'] == true)
                    {
                        ...
                        $themeData['story_url'] = smoothLink('index.php?a=story&id=' . $this->storyId);
                        ...
                        $themeData['mail_generator_avatar'] = $this->timeline['thumbnail_url'];
                        
                        $subject = str_replace("{user}", $this->timeline['name'] . " (@" . $this->timeline['username'] . ")", $lang['new_timelinepost_email_subject']);

                        $message = \SocialKit\UI::view('emails/notifications/new-timeline-post');
                        
                        sendMail($this->recipient['email'], $subject, $message);
                    }
                }
    ...
    public function putNotification()
    {
        ...
        foreach ($this->mentions as $mention)
        {
            ...
            $this->getConnection()->query("INSERT INTO " . DB_NOTIFICATIONS . " (timeline_id,active,notifier_id,post_id,text,time,type,url) VALUES (" . $mention . ",1," . $user['id'] . "," . $this->storyId . ",'$text'," . time() . ",'post_mention','index.php?a=story&id=" . $this->storyId . "')");
        }
        ...
        if (isset($this->recipient['id']))
        {
            if ($this->recipient['type'] == "user")
            {
                ...
                $this->getConnection()->query("INSERT INTO " . DB_NOTIFICATIONS . " (timeline_id,active,notifier_id,post_id,text,time,type,url) VALUES (" . $this->recipient['id'] . ",1," . $user['id'] . "," . $this->storyId . ",'$text'," . time() . ",'timeline_wall_post','index.php?a=story&id=" . $this->storyId . "')");
            }
            elseif ($this->recipient['type'] == "page")
            {
                ...
                foreach ($this->recipientObj->getPageAdmins() as $paId)
                {
                    ...
                    $this->getConnection()->query("INSERT INTO " . DB_NOTIFICATIONS . " (timeline_id,active,notifier_id,post_id,text,time,type,url) VALUES (" . $paId . ",1," . $user['id'] . "," . $this->storyId . ",'$text'," . time() . ",'page_timeline_wall_post','index.php?a=story&id=" . $this->storyId . "')");
                }
            }
        }
    ...
    public function setText($t='')
    {
        ...
        $nbrT = str_replace($tagsToStrip, "", trim($t));

        if (empty($nbrT)) return false;

        global $config;

        if ($config['story_character_limit'] > 0)
        {
            if (strlen($t) > $config['story_character_limit']) return false;
        }
