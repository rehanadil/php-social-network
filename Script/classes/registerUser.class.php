...
class registerUser {
    ...
    public function register()
    {
        if (! empty ($this->name) && ! empty ($this->username) && ! empty ($this->email) && ! empty ($this->password) && ! empty ($this->gender))
        {
            ...
            if ($query)
            {
                ...
                $defaultPlanQuery = $this->getConnection()->query("SELECT id FROM " . DB_SUBSCRIPTION_PLANS . " WHERE is_default=1");
                if ($defaultPlanQuery->num_rows == 1)
                {
                    $defaultPlan = $defaultPlanQuery->fetch_array(MYSQLI_ASSOC);
                    ...
                    $query2 = $this->getConnection()->query("INSERT INTO " . DB_USERS . " ($fields) VALUES ($values)");
    ...
    {
        if (preg_match('/^([-A-Za-z0-9_+.]+)\@([-A-Za-z0-9.]+)$/i', $e))
        {
    ...
    public function setBirthday($b)
    {
        if (is_array($b))
        {
            ...
            {
                $explode = explode('/', $regex);

                if ($explode[2] > (date('Y') - 13)) $this->birthday = $b;
            }
        }
    }
