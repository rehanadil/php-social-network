...
if (isset($story['media']))
{
    ...
    $themeData['post_time'] = $story['time'];
    ...
    if ($story['privacy'] == "friends")
    {
        ...
        {
            $friendsCheck = $conn->query("SELECT id FROM " . DB_FOLLOWERS . " WHERE follower_id=" . $story['timeline']['id'] . " AND following_id=" . $user['id'] . " AND active=1");
    ...
    {
        $themeData['post_view_all_comments_html'] = \SocialKit\UI::view('lightbox/view-all-comments-html');
    }
    ...
    {
        $commentObj = new \SocialKit\Comment();
        $commentObj->setId($v);
        ...
        if (isLogged())
        {
            ...
            {
                $themeData['post_comment_remove_button'] = \SocialKit\UI::view('lightbox/comment-remove-button');
            }
            elseif (! $commentObj->isReported())
            {
                $themeData['post_comment_report_button'] = \SocialKit\UI::view('lightbox/comment-report-button');
            }

            $themeData['post_comment_control_buttons'] = \SocialKit\UI::view('lightbox/comment-control-buttons');
        }

        $themeData['post_comments'] .= \SocialKit\UI::view('lightbox/comment');
    }
    ...
    $data = array(
        'status' => 200,
        'html' => \SocialKit\UI::view('lightbox/content')
    );
