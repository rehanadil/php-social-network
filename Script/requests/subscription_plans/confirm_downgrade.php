...
if (isset($_GET['plan_id']))
{
	...
	if ($planQuery->num_rows == 1)
	{
		...
		$themeData['plan_icon'] = ($plan['plan_icon'] != "") ? $plan['plan_icon'] : $config['theme_url'] . '/images/default-plan-icon.png';
		$themeData['plan_default_icon'] = ($plan['plan_icon'] != "") ? 0 : 1;
		$themeData['is_recurring'] = ($user['stripe_customer_id'] == "") ? 0 : 1;

		$themeData['are_you_sure'] = str_replace('{plan}', '<em>'.$themeData['plan_name'].'</em>', $lang['are_you_sure_you_want_to_downgrade_to_plan']);

		$data = array(
			"status" => 200,
			"html" => \SocialKit\UI::view('subscription-plans/confirm-downgrade')
		);
	}
}
